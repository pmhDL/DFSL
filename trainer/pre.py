...
class PRETrainer(object):
    def __init__(self, args):
        ...
        self.args = args
        # Load pretrain set
        if self.args.addVal == 1:
            flg = True
        else:
            flg = False
        self.trainset = Dataset('train', self.args, T_V=flg)
        self.train_loader = DataLoader(dataset=self.trainset, batch_size=args.pre_batch_size, shuffle=True,
                                       num_workers=8, pin_memory=True)
        # Load meta-val set
        self.valset = Dataset('val', self.args, T_V=flg)
        ...
        if self.args.opt == 'SGD':
            ...
            self.optimizer = torch.optim.Adam([{'params': self.model.encoder.parameters(), 'lr': self.args.pre_lr},
                             {'params': self.model.pre_fc.parameters()}, 
                             {'params': self.model.rot_fc.parameters()}],
